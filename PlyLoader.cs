...
namespace VolPlayer
{
    ...
    public class PlyLoader : MonoBehaviour
    {
        ...
        public static void PrintTime(string suffix = "")
        {
            DateTime dt = DateTime.Now;
            Debug.Log(dt.ToString("yyyy-MM-dd hh:mm:ss.fff") + " " +suffix);
        }
        
        static Point FromBytes(byte[] arr)
        {
            Point str = new Point();
        ...
        public static ComputeBuffer LoadPlyFromBin(string path)
        {
            ...
            using (FileStream stream = new FileStream(path, FileMode.Open)) {
                ...
                int pointCount = streamLength / elementSize;
                byte[] buf = new byte[streamLength];
                stream.Read(buf, 0, (int) streamLength);
            
                var _pointBuffer = new ComputeBuffer(pointCount, elementSize);
                _pointBuffer.SetData(buf);
                // Debug.Log("after deserialize " + pointCount);
                // PrintTime();
                return _pointBuffer;
            }
        }

        public static ComputeBuffer LoadPly(string path)
        {
            const int elementSize = sizeof(float) * 4;
            ...
            Debug.Log("before file open and read");
            var stream = File.Open(path, FileMode.Open, FileAccess.Read, FileShare.Read);
            var header = ReadDataHeader(new StreamReader(stream));
            DataBody body;
            if (header.format == Format.ASCII)
            {
                body = ReadDataBodyASCII(header, new StreamReader(stream));
            } else {
                body = ReadDataBodyBin(header, new BinaryReader(stream));
            }
            Debug.Log("complete file open and read");

            positions = body.vertices;
            colors = body.colors;
            ...
            for (var i = 0; i < _pointData.Length; i++)
            {
                ...
                // Debug.Log(_pointData[i].position + _pointData[i].color.ToString());
            }
            Debug.Log("Point Count: "+positions.Count);
            //Debug.Log(_pointData[100].position);
            _pointBuffer = new ComputeBuffer(_pointData.Length, elementSize);
            _pointBuffer.SetData(_pointData);
            return _pointBuffer;
            
        }

        public void LoadPlyToRenderer()
        {
            ComputeBuffer buf = LoadPly(PlyFilePath);
            PointCloudRenderer pointCloudRenderer = GetComponent<PointCloudRenderer>();
            pointCloudRenderer.sourceBuffer = buf;
        }
        ...
        static DataHeader ReadDataHeader(StreamReader reader)
        {
            ...
            {
                data.format = Format.ASCII;
            }
            else
            {
                data.format = Format.Binary;
            }
            readCount += line.Length + 1;
        ...
        static public DataBody ReadDataBodyASCII(DataHeader header, StreamReader reader)
        {
            ...
            for (var i = 0; i < header.vertexCount; i++)
            {
                ...
                {
                    //Debug.Log(Enum.GetName(typeof(DataProperty), prop) + " " + col[j]);
                    switch (prop)
                    {
                        case DataProperty.R8: r = (byte)int.Parse(col[j]); break;
                        case DataProperty.G8: g = (byte)int.Parse(col[j]); break;
                        case DataProperty.B8: b = (byte)int.Parse(col[j]); break;
                        case DataProperty.A8: a = (byte)int.Parse(col[j]); break;
                        ...
                        case DataProperty.SingleX: x = float.Parse(col[j]); break;
                        case DataProperty.SingleY: y = float.Parse(col[j]); break;
                        case DataProperty.SingleZ: z = float.Parse(col[j]); break;

                        case DataProperty.DoubleX: x = float.Parse(col[j]); break;
                        case DataProperty.DoubleY: y = float.Parse(col[j]); break;
                        case DataProperty.DoubleZ: z = float.Parse(col[j]); break;
        ...
        public static DataBody ReadDataBodyBin(DataHeader header, BinaryReader reader)
        {
            var data = new DataBody(header.vertexCount);
        ...
        static uint EncodeColor(Color c)
        {
            ...
            // Debug.Log(c.r+ " " + c.g + " " + c.b + " " + y);
            var rgb = new Vector3(c.r, c.g, c.b);
            rgb *= 255 * 255 / (y * kMaxBrightness);
            // Debug.Log((uint)rgb.x + " " + ((uint)rgb.y) + " " + ((uint)rgb.z) + " " + y);
    ...
    #if UNITY_EDITOR
    [CustomEditor(typeof(PlyLoader))]
    class PlyLoaderEditor : Editor
    {
        private PlyLoader loaderInstance;
        
        private void Awake()
        {
            loaderInstance = (PlyLoader)target;
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                loaderInstance.LoadPlyToRenderer();
            }
        }
